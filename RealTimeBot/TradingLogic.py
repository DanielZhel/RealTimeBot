...
def get_short_entry(macd_values, ema_values, stoch_values, inst_data2, fig, go, instId):
    
    ema_min = ema_values[0][98]-ema_values[0][98]*(0.6/100)
    if(ema_min<=inst_data2['open'][298]<=ema_values[0][98]):
        if(macd_values[0][0][98]<macd_values[0][0][97] and macd_values[0][1][98]<macd_values[0][1][97]):
            if (stoch_values[0][297] >70 and stoch_values[0][298] < stoch_values[1][298] and stoch_values[0][297]>stoch_values[1][297]):
                ...
                with open('ShortStat.txt', 'w') as f:
                    f.write(f'{instId}, time:{time}, price: {entry}\n')
                chime.info()
                      
        
def get_long_entry(macd_values, ema_values, stoch_values, inst_data2, fig, go, instId):
